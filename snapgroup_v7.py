...
def create_volist(vlist):
    ...
    for volume in vlist:
        ...
        if v.json()['result']:
            ...
            logging.info('{} not found, stopping...'.format(volume))
            exit(1)
    return vol_list
...
def create_snapgroup(vol_list):
    ...
    if outp.status_code == 200:
        ...
        logging.info("Snap group created successfully")
    else:
        print('snap group creation failed: {}'.format(outp.json()['error']['message']))
        logging.info('snap group creation failed: {}'.format(outp.json()['error']['message']))
        exit(outp.status_code)
...
def del_snapgroup(sgname, choice=None):
    ...
    if del_snap_list:
        ...
        logging.info("snap group not found")
        exit(1)
            ...
            if args.option == 'create':
                ...
                if args.volumes and args.name:
                    ...
                    with redirect_stdout(trap):
                        snaps=get_snapgroup(args.name[0])
                        if len(snaps) >= int(nsnaps):
                            del_snapgroup(args.name[0], 1)
